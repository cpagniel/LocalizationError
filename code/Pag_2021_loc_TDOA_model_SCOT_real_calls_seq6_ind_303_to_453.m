...
% Grid-search algorithm using modeled time-differences
...
for gcnt = 1:length(event.start_time)
    ...
    if event.fishID(gcnt) == 1
        data.extend1 = 0; % import 0 seconds before logged call for SNR measurements
        data.extend2 = 0; % import 0 seconds after logged call for SNR measurements
    elseif event.fishID(gcnt) == 2
        data.extend1 = 0; % import 0.025 seconds before logged call for SNR measurements
    ...
    PARAMS.ch = ind;
    if TEMP.start_ind(ind)/data.fs-0.02 >= 0
        PARAMS.start_ind = round(TEMP.start_ind(ind)-0.02.*data.fs);
    else
        PARAMS.start_ind = 1;
    end
