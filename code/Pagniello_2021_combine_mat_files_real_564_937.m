...
for i = 1:length(files) %1:length(files)
    ...
    start_time(i) = event.start_time(563+i) + seconds(data.extend1) + seconds(PARAMS.start_ind./data.fs); %seconds(start_sample(i)./48000)+datetime(wavname2dnum_edit(Log{i,1}),'ConvertFrom','datenum');
...
save(['F:\C_files\C_Spring_2021\Real_Call_Localization\Localization_Output\',...
    'typeUF1_combo_SCOT_564_to_937_interp_5m_error_5m_update.mat'],...
    'xx','yy','zz',...
    'MAX_C','MTDOA',...
    'RL*','SL*','BW*','f*','DELTA_t',...
    'start*','grid_type');
