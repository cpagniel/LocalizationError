...
for i = 1:length(files) %1:length(files)
    ...
    start_time(i) = event.start_time(i) + seconds(data.extend1) + seconds(PARAMS.start_ind./data.fs); %seconds(start_sample(i)./48000)+datetime(wavname2dnum_edit(Log{i,1}),'ConvertFrom','datenum');
    
    fishID(i) = event.fishID(i);
    
    clearvars data event position mTDOA max_C PARAMS
...
save(['F:\C_files\C_Spring_2021\Real_Call_Localization\Localization_Output\',...
    'typeUF1_combo_SCOT_303_to_453_interp_5m_error_5m.mat'],...
    'xx','yy','zz',...
    'MAX_C','MTDOA',...
    'RL*','SL*','BW*','f*','DELTA_t',...
    'start*','fishID');
