...
clear LOG* event ind* GPS* x y round_z
...
for gcnt = [1:1109 1527:length(event.start_time)]
    ...
    data.extend = 0.005; % import 0.005 seconds before and after logged call for SNR measurements
    ...
    [data.raw,~] = audioread(char(Log{gcnt,1}),[data.start_sample-(data.extend*data.fs) data.end_sample+(data.extend*data.fs)],'native'); % event data raw audio from start to end of logged sound
    ...
    Nfft = 512; overlap = 0.9;
    
    for m = 1:array.M
        
        TEST.start_ind(m) = find(cumsum(data.filt(:,m).^2) <= ...
            max(cumsum(data.filt(:,m).^2))*0.05,1,'last');
        TEST.end_ind(m) = find(cumsum(data.filt(:,m).^2) <= ...
            max(cumsum(data.filt(:,m).^2))*0.95,1,'last');
        
        TEST.dur(m) = (TEST.end_ind(m) - TEST.start_ind(m))./data.fs;
            
        [Pxx,f] = pwelch(data.filt(TEST.start_ind(m):TEST.end_ind(m),m),kaiser(Nfft,7.85),...
        ...
        [TEST.BW3dB(m),TEST.flow3dB(m),TEST.fhigh3dB(m)] = powerbw(Pxx_interp,f_interp,[],3);
        [TEST.BW6dB(m),TEST.flow6dB(m),TEST.fhigh6dB(m)] = powerbw(Pxx_interp,f_interp,[],6);
        [TEST.BW10dB(m),TEST.flow10dB(m),TEST.fhigh10dB(m)] = powerbw(Pxx_interp,f_interp,[],10);
        
        if TEST.flow10dB(m) == 0
            TEST.flow10dB(m) = 1;
        end
        
        bpFilt = designfilt('bandpassiir','FilterOrder',8,...
            'PassbandFrequency1',TEST.flow10dB(m),'PassbandFrequency2',TEST.fhigh10dB(m), ...
            'PassbandRipple',1,'SampleRate',data.fs); % 8th-order Band Pass IIR
        
        TEST.data = filter(bpFilt,data.dmean(TEST.start_ind(m):TEST.end_ind(m),m)); % filter data
        
        TEST.RL_pp_dB(m) = 20*log10(max(TEST.data)-min(TEST.data));
        TEST.RL_rms_dB(m) = 20*log10(sqrt(mean(TEST.data.^2)));
        
        clear bpFilt
  
    end  
    
    clear Pxx* f* Nfft overlap ind m
    
    % Re-compute Call Parameters for Channel with Highest RL peak-to-peak
    
    [~,ind] = max(TEST.RL_pp_dB);
    
    PARAMS.ch = ind;
    PARAMS.start_ind = TEST.start_ind(ind);
    PARAMS.end_ind = TEST.end_ind(ind);
    PARAMS.dur = TEST.dur(ind);
    
    PARAMS.fpeak = TEST.fpeak(ind);
    
    PARAMS.BW3dB = TEST.BW3dB(ind); PARAMS.flow3dB = TEST.flow3dB(ind); PARAMS.fhigh3dB = TEST.fhigh3dB(ind);
    PARAMS.BW6dB = TEST.BW6dB(ind); PARAMS.flow6dB = TEST.flow6dB(ind); PARAMS.fhigh6dB = TEST.fhigh6dB(ind);
    PARAMS.BW10dB = TEST.BW10dB(ind); PARAMS.flow10dB = TEST.flow10dB(ind); PARAMS.fhigh10dB = TEST.fhigh10dB(ind);
    
    PARAMS.RL_pp_dB = TEST.RL_pp_dB(ind); PARAMS.RL_rms_dB = TEST.RL_rms_dB(ind);
    ...
    max_acor = zeros(nchoosek(array.M,2),1);
    
    cnt = 1;
    for m = 1:array.M-1
        for n = m+1:array.M
            [acor,lag] = SCOT(data.signal(:,n),data.signal(:,m),[PARAMS.flow10dB PARAMS.fhigh10dB],data.fs);
            [~,ind] = max(abs(acor));
            lagDiff = lag(ind);
            mTDOA(cnt) = lagDiff/data.fs;
            max_acor(cnt) = max(abs(acor));
    ...
    save(['F:\C_files\C_Spring_2021\Real_Call_Localization\Localization_Output\UF1',...
        ...
        'cTDOA','mTDOA','max_acor','LS','position','event','data','elapsed_time','PARAMS');
