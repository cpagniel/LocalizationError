...
Log = readtable('D:\Localization_Experiment\805572635\wav\Two_fish_calling_09062019_start_211045.xls'); % importing the log
...
for i = 1:length(Log{:,4})
    if strcmp(Log{i,4},'UF1') == 1
        event.fishID(i,1) = 1;
    else
        event.fishID(i,1) = 2;
    end
...
for gcnt = 1:length(event.start_time)
    ...
    if event.fishID(gcnt) == 1
        data.extend1 = 0.02; % import 0 seconds before logged call for SNR measurements
        data.extend2 = 0; % import 0 seconds after logged call for SNR measurements
    elseif event.fishID(gcnt) == 2
        data.extend1 = 0.02; % import 0.025 seconds before logged call for SNR measurements
        data.extend2 = 0; % import 0seconds after logged call for SNR measurements
    end
    ...
    for m = 1:array.M
        ...
        axis square
        title(num2str(event.fishID(gcnt)));
    ...
    %% Save in PARAMS Parameters from Channel 3 - routinely the cleanest
    % channel
    ...
    if TEMP.start_ind(ind)/data.fs-extend >= 0
        for m = 1:array.M
            
            figure(1); subplot(2,2,m);
            hold on
            %         plot([min(TEMP.start_ind) min(TEMP.start_ind)]./data.fs,[min(ylim) max(ylim)],'w--','LineWidth',2);
            %         plot([max(TEMP.end_ind) max(TEMP.end_ind)]./data.fs,[min(ylim) max(ylim)],'w--','LineWidth',2);
            ...
            plot([TEMP.start_ind(ind) TEMP.start_ind(ind)]./data.fs-extend1,[min(ylim) max(ylim)],'w--','LineWidth',2);
            plot([TEMP.end_ind(ind) TEMP.end_ind(ind)]./data.fs+extend2,[min(ylim) max(ylim)],'w--','LineWidth',2);
            plot([min(xlim) max(xlim)],[TEMP.flow10dB(ind) TEMP.flow10dB(ind)],'w--','LineWidth',2);
            plot([min(xlim) max(xlim)],[TEMP.fhigh10dB(ind) TEMP.fhigh10dB(ind)],'w--','LineWidth',2);
            
            figure(2); subplot(2,2,m);
            hold on
            %         plot(time(min(TEMP.start_ind):max(TEMP.end_ind)),data.filt(min(TEMP.start_ind):max(TEMP.end_ind),m)/10^6,'b-')
            plot(time(round(TEMP.start_ind(ind)-extend1.*data.fs):round(TEMP.end_ind(ind)+extend2.*data.fs)),data.filt(round(TEMP.start_ind(ind)-extend1.*data.fs):round(TEMP.end_ind(ind)+extend2.*data.fs),m)/10^6,'r-')
        ...
        for m = 1:array.M
            ...
            plot([1 1],[min(ylim) max(ylim)],'w--','LineWidth',2);
            plot([TEMP.end_ind(ind) TEMP.end_ind(ind)]./data.fs+extend2,[min(ylim) max(ylim)],'w--','LineWidth',2);
            plot([min(xlim) max(xlim)],[TEMP.flow10dB(ind) TEMP.flow10dB(ind)],'w--','LineWidth',2);
            plot([min(xlim) max(xlim)],[TEMP.fhigh10dB(ind) TEMP.fhigh10dB(ind)],'w--','LineWidth',2);
            ...
            hold on
            %         plot(time(min(TEMP.start_ind):max(TEMP.end_ind)),data.filt(min(TEMP.start_ind):max(TEMP.end_ind),m)/10^6,'b-')
            plot(time(1:round(TEMP.end_ind(ind)+extend2.*data.fs)),data.filt(1:round(TEMP.end_ind(ind)+extend2.*data.fs),m)/10^6,'r-')
    ...
    PARAMS.ch = ind;
    if TEMP.start_ind(ind)/data.fs-extend1 >= 0
        PARAMS.start_ind = round(TEMP.start_ind(ind)-extend1.*data.fs);
    else
        PARAMS.start_ind = 1;
    end
