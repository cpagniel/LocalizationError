...
for gcnt = 1:length(event.start_time)
    ...
    if event.fishID(gcnt) == 1
        data.extend1 = 0.05; % import 0 seconds before logged call for SNR measurements
        data.extend2 = 0.05; % import 0 seconds after logged call for SNR measurements
    elseif event.fishID(gcnt) == 2
        data.extend1 = 0.05; % import 0.025 seconds before logged call for SNR measurements
        data.extend2 = 0.05; % import 0seconds after logged call for SNR measurements
    end
    ...
    bpFilt = designfilt('bandpassiir','FilterOrder',8,...
        'PassbandFrequency1',event.freq_min(gcnt)-100,'PassbandFrequency2',1500, ...
    ...
    %% Click Start and End Times
    
    [temp_x,~] = ginput(2);
    TEMP.manual_start_time = temp_x(1);
    TEMP.manual_end_time = temp_x(2);
    
    figure(1);
    hold on
    
    plot([TEMP.manual_start_time TEMP.manual_start_time],[min(ylim) max(ylim)],'w--','LineWidth',2);
    plot([TEMP.manual_end_time TEMP.manual_end_time],[min(ylim) max(ylim)],'w--','LineWidth',2);
    
    saveas(figure(1),['log_no' num2str(gcnt) '_spec_manual.fig']); saveas(figure(1),['log_no' num2str(gcnt) '_manual_spec.png']);
    close all
    
    %% Save Parameters from Channel 3 and Manual Start and End Times
    
    ind = 3;
    
    PARAMS.ch = ind;
    PARAMS.start_ind = floor(TEMP.manual_start_time.*data.fs);
    PARAMS.end_ind = ceil(TEMP.manual_end_time.*data.fs);
    PARAMS.dur = TEMP.manual_end_time-TEMP.manual_start_time;
    ...
    save(['F:\C_files\C_Spring_2021\Real_Call_Localization\Localization_Output\UF1',...
        '\log_no' num2str(gcnt) '\log_no' num2str(gcnt) '_combo_SCOT_interp_5m_error_5m_manual.mat'],...
        'cTDOA','mTDOA','max_C','LS','position','event','data','elapsed_time','TEMP','PARAMS','gridd');
    ...
    saveas(gcf,['log_no' num2str(gcnt) '_LS_combo_SCOT_interp_5m_error_5m_manual.fig']);
    saveas(gcf,['log_no' num2str(gcnt) '_LS_combo_SCOT_interp_5m_error_5m_manual.png']);
