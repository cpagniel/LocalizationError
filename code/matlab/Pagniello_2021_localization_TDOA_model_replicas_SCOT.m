...
% Grid-search algorithm uses modeled time-differences of arrival (TDOAs) to 
...
for gcnt = 1:length(event_no)
    ...
    %% Select Same Search Area as Measured Replicas
    
    load('F:\C_files\C_Spring_2021\Localization_Playbacks\mTDOA_SCOT_for_type0.mat','F_s12','F_d12');
    ...
    save(['F:\C_files\C_Spring_2021\Localization_Playbacks\Localization_Output\',...
        num2str(event.freqID) '\event_no' num2str(event_no(gcnt)) '\event_no' num2str(event_no(gcnt)) '_model_SCOT.mat'],...
        'mTDOA','max_C','LS','position','error','event','data','elapsed_time','PARAMS');
    ...
    saveas(gcf,['event_no' num2str(event_no(gcnt)) '_LS_model_SCOT.fig']);
    saveas(gcf,['event_no' num2str(event_no(gcnt)) '_LS_model_SCOT.png']);
