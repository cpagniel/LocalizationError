...
for gcnt = 564:937 %1:length(event.start_time)
    ...
    %% Save in PARAMS Parameters from Channel 2
    ...
    if TEMP.start_ind(ind_global)/data.fs-0.02 >= 0
        for m = 1:array.M
            ...
            plot([TEMP.start_ind(ind_global) TEMP.start_ind(ind_global)]./data.fs-0.02,[min(ylim) max(ylim)],'w--','LineWidth',2);
            plot([TEMP.end_ind(ind_global) TEMP.end_ind(ind_global)]./data.fs+0.02,[min(ylim) max(ylim)],'w--','LineWidth',2);
            plot([min(xlim) max(xlim)],[TEMP.flow10dB(ind_global) TEMP.flow10dB(ind_global)],'w--','LineWidth',2);
            plot([min(xlim) max(xlim)],[TEMP.fhigh10dB(ind_global) TEMP.fhigh10dB(ind_global)],'w--','LineWidth',2);
            
            figure(2); subplot(2,2,m);
            hold on
            %         plot(time(min(TEMP.start_ind):max(TEMP.end_ind)),data.filt(min(TEMP.start_ind):max(TEMP.end_ind),m)/10^6,'b-')
            plot(time(round(TEMP.start_ind(ind_global)-0.02.*data.fs):round(TEMP.end_ind(ind_global)+0.02.*data.fs)),data.filt(round(TEMP.start_ind(ind_global)-0.02.*data.fs):round(TEMP.end_ind(ind_global)+0.02.*data.fs),m)/10^6,'r-')
            
            figure(3); subplot(2,2,m);
            hold on
            plot([TEMP.fpeak(ind_global) TEMP.fpeak(ind_global)],[min(ylim) max(ylim)],'k--','LineWidth',2);
            plot([TEMP.flow10dB(ind_global) TEMP.flow10dB(ind_global)],[min(ylim) max(ylim)],'k-','LineWidth',2);
            plot([TEMP.fhigh10dB(ind_global) TEMP.fhigh10dB(ind_global)],[min(ylim) max(ylim)],'k-','LineWidth',2);
        ...
        for m = 1:array.M
            ...
            hold on
            %         plot(time(min(TEMP.start_ind):max(TEMP.end_ind)),data.filt(min(TEMP.start_ind):max(TEMP.end_ind),m)/10^6,'b-')
            plot(time(1:round(TEMP.end_ind(ind_global)+0.02.*data.fs)),data.filt(1:round(TEMP.end_ind(ind_global)+0.02.*data.fs),m)/10^6,'r-')
